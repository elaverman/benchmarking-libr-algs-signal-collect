...
import com.signalcollect.dcopthesis.GlobalStats
...
class EvaluationSuiteCreator(evaluationName: String,
  ...
  def buildEvaluationJob(run: EvaluationAlgorithmRun[_, _], extraInformation: Map[String, String]): TorqueJob = new TorqueJob(
    ...
    execute = { () =>
      ...
      GlobalStats.currentRunNumber += 1
      println(s"run ${GlobalStats.currentRunNumber} of ${GlobalStats.totalNumberOfRuns}")
      ...
      /*
       * Logging of stats for benchmark that are available
       * only after execution
       * The same keys have to be present in the CSVResultHandlers value extractor list
       */
      val ratios = GlobalStats.utilityRatios.toSeq
      val stepsToGlobalOptimum = GlobalStats.timeToOptimum
      val stepsToNashEquilibirium = GlobalStats.timeToNashEquilibrium

      statsMap += (("timeToOptimum", stepsToGlobalOptimum.toString))
      statsMap += (("timeToNashEq", stepsToNashEquilibirium.toString))
      statsMap += (("ratioAtTermination", ratios.last.toString))
      //statsMap += (("avgRatio", (ratios.sum/ratios.size).toString))

      val totalMessagesReceived = stats.aggregatedWorkerStatistics.messagesReceived
      val collectOperationsExecuted = stats.aggregatedWorkerStatistics.collectOperationsExecuted
      val signalOperationsExecuted = stats.aggregatedWorkerStatistics.signalOperationsExecuted

      statsMap += (
          ("avgNumberMessagesRecievedPerCycle",
          (totalMessagesReceived/collectOperationsExecuted.toDouble).toString))

      statsMap += (("ratios", ratios.mkString(",")))

      // Clear stats for logging of next run
      GlobalStats.clear()
